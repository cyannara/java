...
//메인 클래스 입니다. 나중을 위해 단순 프레임 생성은
//별도의 클래스에서 만들어 불러 오도록 하죠.
  Frame_make_sub fms = new Frame_make_sub();
 }
}

class Frame_make_sub extends JFrame{ // 프레임을 만들기 위한 클래스 입니다.

 int f_width = 800; //생성할 프레임의 넓이 값을 설정합니다.
 int f_height = 600; //생성할 프레임의 높이 값을 설정합니다.
 
 Frame_make_sub(){ //프레임을 만듭니다.
  init(); //나중을 위한 프레임에 들어갈 컴포넌트 세팅 메소드입니다.
  start(); //나중을 위한 기본적인 시작 명령 처리 부분입니다.
  
  setTitle("프레임 테스트"); //프레임의 이름을 설정합니다.
